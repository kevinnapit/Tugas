...
File ini digunakan untuk mendefinisikan "blueprint" atau cetakan dari sebuah item menu.

```java
/*
 * File: Menu.java
 * Penjelasan:
 * Ini adalah kelas sederhana (POJO - Plain Old Java Object) yang digunakan
 * untuk merepresentasikan satu item menu di restoran.
 * Kelas ini hanya berisi data (atribut) dan sebuah constructor
 * untuk menginisialisasi data tersebut.
 */
public class Menu {

    // --- Atribut ---
    // Atribut ini akan menyimpan informasi untuk setiap objek Menu.

    String nama;      // Untuk menyimpan nama menu (misal: "Nasi Goreng")
    double harga;     // Untuk menyimpan harga menu (misal: 25000.0)
    String kategori;  // Untuk menyimpan kategori (misal: "makanan" atau "minuman")

    /*
     * --- Constructor ---
     * Penjelasan:
     * Constructor adalah method khusus yang dipanggil saat kita membuat
     * objek baru dari kelas ini (menggunakan kata kunci 'new').
     * Gunanya adalah untuk "memasukkan" nilai awal ke dalam atribut
     * yang sudah kita definisikan di atas.
     */
    public Menu(String nama, double harga, String kategori) {
        // 'this.nama' merujuk ke atribut 'nama' milik kelas ini.
        // 'nama' (tanpa 'this') merujuk ke parameter yang dikirimkan.
        this.nama = nama;
        this.harga = harga;
...
File ini berisi semua logika program, seperti menampilkan menu, memproses pesanan, menghitung total, dan mencetak struk.

```java
/*
 * File: Main.java
 * Penjelasan:
 * Ini adalah kelas utama yang akan menjalankan seluruh alur program.
 * Kelas ini berisi method main() dan method-method pembantu lainnya
 * untuk mengelola aplikasi restoran.
 */

// Mengimpor kelas Scanner untuk bisa menerima input dari pengguna.
import java.util.Scanner;

public class Main {

    /*
     * --- Method main (Metode Utama) ---
     * Penjelasan:
     * Ini adalah titik awal eksekusi program Java.
     * Semua alur logika utama akan dimulai dari sini.
     */
    public static void main(String[] args) {

        // --- 1. Inisialisasi Data Menu ---
        // Penjelasan:
        // Kita membuat array bernama 'daftarMenu' yang bisa menampung 8 objek 'Menu'.
        // Array adalah cara menyimpan banyak data sejenis dalam satu variabel.
        // Sesuai permintaan, kita tidak menggunakan loop, jadi kita isi array
        // satu per satu secara manual (hardcode).
        Menu[] daftarMenu = new Menu[8];

        // Mengisi array dengan data Makanan (Minimal 4)
        ...
        // Menu makanan ini harganya mahal untuk mempermudah tes skenario > 100rb
        daftarMenu[3] = new Menu("Soto Betawi", 30000, "makanan");

        // Mengisi array dengan data Minuman (Minimal 4)
        ...
        // --- 3. Menampilkan Menu ---
        // Memanggil method 'tampilkanMenu' yang sudah kita buat di bawah
        // untuk menunjukkan daftar menu ke pengguna.
        tampilkanMenu(daftarMenu);


        // --- 4. Proses Pemesanan (Maksimal 4 item) ---
        // Penjelasan:
        // Sesuai permintaan, kita tidak boleh pakai loop.
        // Jadi, kita akan menanyakan pesanan satu per satu (maksimal 4 kali)
        // dan menyimpannya di variabel yang terpisah (nama1, qty1, nama2, qty2, dst.).
        ...
        int qty1 = 0; // Inisialisasi jumlah pesanan 1
        // Struktur Keputusan: Hanya tanya jumlah jika nama menunya BUKAN 'stop'
        if (!nama1.equalsIgnoreCase("stop")) {
            System.out.print("Masukkan Jumlah: ");
            qty1 = scanner.nextInt();
            scanner.nextLine(); // Penting: Membersihkan 'enter' dari buffer scanner
        }
        ...
        scanner.close(); // Menutup scanner karena sudah tidak dipakai

        // --- 5. Mencari Objek Menu Berdasarkan Nama ---
        // Penjelasan:
        // Kita memanggil method 'cariMenu' untuk mendapatkan objek Menu
        // (yang berisi harga dan kategori) berdasarkan nama yang di-input pengguna.
        ...
        // Penjelasan:
        // Di sinilah logika bisnis inti (diskon, pajak, dll) diterapkan
        // menggunakan struktur keputusan (if, if-else).
        ...
        // Hitung total per item dan tambahkan ke SubTotal
        // Struktur Keputusan: Hanya hitung jika menu-nya valid (ditemukan) DAN jumlahnya > 0
        ...
        // --- Skenario Struktur Keputusan 1: Diskon 10% ---
        // Penjelasan:
        // Jika total biaya (subTotal) melebihi Rp 100.000,
        // berikan diskon 10% dari subTotal.
        if (subTotal > 100000) {
            diskon = subTotal * 0.10;
        }


        // --- Skenario Struktur Keputusan 2: Penawaran BOGO ---
        // Penjelasan:
        // Jika total biaya (subTotal) melebihi Rp 50.000,
        // berikan gratis 1 untuk minuman PERTAMA yang dipesan.
        // Kita harus cek pesanan 1, 2, 3, 4 secara berurutan.
        if (subTotal > 50000) {
        ...
        // --- 7. Mencetak Struk ---
        // Penjelasan:
        // Memanggil method 'cetakStruk' dan mengirimkan SEMUA data
        // yang dibutuhkan untuk ditampilkan di struk.
        cetakStruk(
                menu1, qty1, total1,
                menu2, qty2, total2,
                menu3, qty3, total3,
     ...
     * Penjelasan:
     * Method ini bertugas menampilkan isi dari 'daftarMenu' ke layar.
     * Sesuai permintaan, kita tidak pakai loop. Kita tampilkan manual
     ...
     * --- Method cariMenu ---
     * Penjelasan:
     * Method ini mencari objek Menu di dalam array 'daftarMenu'
     * berdasarkan 'nama' yang dicari.
     * Sesuai permintaan, kita tidak pakai loop, jadi kita gunakan
     * struktur keputusan 'if-else if' (percabangan) untuk mengecek
     * satu per satu isi array.
     */
     ...
     * --- Method cetakStruk ---
     * Penjelasan:
     * Method ini bertugas HANYA untuk menampilkan struk ke layar.
     * Method ini menerima banyak parameter hasil perhitungan dari method main.
     * Method ini juga menggunakan struktur keputusan 'if' untuk
     * HANYA menampilkan item yang dipesan (qty > 0) dan
     * HANYA menampilkan diskon jika memang ada (diskon > 0).
     */
    public static void cetakStruk(
        ...
        // Struktur Keputusan: Tampilkan pesanan 1 HANYA JIKA dipesan
        if (m1 != null && q1 > 0) {
